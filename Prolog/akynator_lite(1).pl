...
question5(X8):-	write("Does your language require semicolons at the end of each statement?"),nl,
				write("1. Yes"),nl,
				write("0. NO"),nl,
				read(X8).
...
pr:-	question1(X1),question2(X2),question3(X3),question4(X4),
		...
		cross(X,X5),visual(X,X6),mobile(X,X7),semicolon(X,X8),
		write(X).